...
module.exports = class SimpleNN {
    constructor() {
        ...
        How to run it :
        let dqn = new DQN();
        let values = dqn.getDataset();
        await dqn.TrainData(values[0], values[1]);
        values = dqn.getDataset();
        values[0] = values[0].slice(210,220);
        let predictions = dqn.Predict(values[0]);
